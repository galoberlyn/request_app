...
$objSheet->getStyle('B10:K10')->getFont()->setBold(true)->setSize(12);
...
if($query_check_arr['type'] == 'PO'){
	...
	$POquery = "SELECT description,quantity,amount,remarks,date_complete,status, supplier_po, Location, serial_number, model FROM request_slip INNER JOIN purchase_order ON request_slip.id = request_id Inner JOIN itemspo ON purchase_order.id = poid where request_slip.id = ?;";
	...
	$objSheet->getCell('D10')->setValue('Unit Price');
	$objSheet->getCell('E10')->setValue('Total Amount');
	$objSheet->getCell('F10')->setValue('Item Status');
	$objSheet->getCell('G10')->setValue('Remarks');
	$objSheet->getCell('H10')->setValue('Date Delivered');
	$objSheet->getCell('I10')->setValue('Location');
	$objSheet->getCell('J10')->setValue('Serial Number');
	$objSheet->getCell('K10')->setValue('Model');
	...
	for($i=1; $table = $POquery_result -> fetch_array(MYSQLI_ASSOC); $i++){
		...
		$objSheet->getCell('D1'.$i)->setValue($table['supplier_po']); //date delivered dapat
		...
		$objSheet->getCell('H1'.$i)->setValue($table['date_complete']);
		$objSheet->getCell('I1'.$i)->setValue($table['Location']);
		$objSheet->getCell('J1'.$i)->setValue($table['serial_number']);
		$objSheet->getCell('K1'.$i)->setValue($table['model']);
	}
	...
	$objSheet->getCell('C10')->setValue('Status');
	...
	$objSheet->getCell('G10')->setValue('Remarks');
	$objSheet->getCell('H10')->setValue('Date Delivered');
	$objSheet->getCell('I10')->setValue('Serial Number');
	$objSheet->getCell('J10')->setValue('Model');
	...
	for($i=1; $table = $POquery_result -> fetch_array(MYSQLI_ASSOC); $i++){
		...
		$objSheet->getCell('B1'.$i)->setValue($table['description']);
		$objSheet->getCell('C1'.$i)->setValue($table['quantity']);
		$objSheet->getCell('D1'.$i)->setValue($table['supplier']); //date delivered dapat
		$objSheet->getCell('E1'.$i)->setValue($table['amount']); 
		$objSheet->getCell('F1'.$i)->setValue($table['status']);
		$objSheet->getCell('G1'.$i)->setValue($table['remarks']);
		$objSheet->getCell('H1'.$i)->setValue($table['date_accomplished']);
		$objSheet->getCell('I1'.$i)->setValue($table['serial_number']);
		$objSheet->getCell('J1'.$i)->setValue($table['model']);
	}
}
// write header
