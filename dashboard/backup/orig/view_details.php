...
if($reqSlipArr['type'] == 'PO'){
  ...
  $items = "SELECT iditemspo,description,supplier_po,quantity,Location,unitprice,itemspostatus,date_complete,remarks, iditemspo, qty_delivered_po, serial_number, model from itemspo where poid = ?";
  ...
  $items = "SELECT id,description,quantity,supplier,date_accomplished,amount,itemStatus,remarks, qty_delivered_nopo, serial_number, model from itemsnotpo where request_slip_no = ?";
  $itemsQry=$conn->prepare($items);
...
<?php
if($reqSlipArr['type'] == 'PO'){
  $itemsQry->bind_result($iditemspo,$description, $supplier_po, $quantity, $location, $price, $itemspostatus, $date_complete, $remarks, $iditemspo, $qty_delivered_po, $serial_number, $model);
        ...
        <div class="modal-header">
          ...
          <form onsubmit="return confirm('Are you sure? this action cannot be undone');" method="post" class='form-horizontal' action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']);?>">
           <select id='dropdown_additional' name='items_additional'>
            ...
            <?php 
            if($reqSlipArr['type']=='PO'){
              $optionselect="SELECT description FROM itemspo Where poid =  $poArr[id] AND (remarks is NULL OR Location is NULL OR amount=0 OR unitprice=0)";
          ...
          <div class='row'>
            ...
            <div class='col-lg-7'>
            <input class='form-control' type="number" name="itemprice" max="10000" required> 
            </div>
          </div><?php echo "<br>"; } 
          ...
          <div class='col-lg-7'>
          <input class='form-control' type="text" name="itemremarks" required><br>
          </div>
          </div>
...
if(isset($_POST['addition_items'])){
  ...
  if($reqSlipArr['type']=='PO'){
    ...
    $model_po = $_POST['model_po'];
    $serial_po = $_POST['serial_po'];
    $date_completed_po= date('Y-m-d', strtotime($_POST['date_completed_po']));

    $selector = "SELECT quantity from itemspo where poid=? AND description=?";
    $selector_r=$conn->prepare($selector);
    $selector_r->bind_param('is', $poArr['id'], $items_additional);
    $selector_r->execute();
    $selector_arr=$selector_r->get_result()->fetch_array(MYSQLI_ASSOC);

    $amount=$selector_arr['quantity'] * $itemprice;


    $additionalPO = "UPDATE itemspo set amount=?, Location=?, remarks=?, serial_number=?, model=?, date_complete=?, unitprice=? where poid=? AND description=?";
    $additionalPO_r=$conn->prepare($additionalPO);
    $additionalPO_r->bind_param('issssssis', $amount, $location, $itemremarks, $serial_po, $model_po, $date_completed_po, $amount, $poArr['id'], $items_additional);
   ...
   echo "<html><head><script> var div = document.createElement('div');
    ...
    $items_additional=$_POST['items_additional'];
    $amount=$_POST['amount'];
    $serial_no_po = $_POST['serial_no_po'];
    $model_no_po = $_POST['model_no_po'];
...
<br>
  ...
  <legend>Add Delivery
    ...
    <form onsubmit="return confirm('Are you sure? this action cannot be undone');" method='POST'>
